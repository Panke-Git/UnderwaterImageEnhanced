"""
    ...
    @Author: Panke
    @FileName: Unet_HybridAttention2_V10.py
    @Time: 2025/6/12 23:21
    @Email: None
"""
...
class UNetHybridAttention2V11(nn.Module):
    """
    使用HybridAttention2, 优化高频的部分，插入到Unet的第三层；
    """

    def __init__(self, in_channels=3, out_channels=3, base_c=64):
        super(UNetHybridAttention2V11, self).__init__()
        self.model_name = 'UNetHybridAttention2V11'
...
class HybridAttention(nn.Module):
    def __init__(self, dim, threshold=0.05):
        ...
        # self.ll_enhance = LLEnhancer(channel=dim)
        self.alpha = nn.Parameter(torch.zeros(dim, 1, 1))
        self.hf_enhance = HFEnhancer_DWT(dim)
    ...
    def forward(self, x):
        ...
        Yl, Yh = self.dwt(x)
        ll = Yl * self.alpha
        # ll = self.ll_enhance(Yl)
        Yh = self.hf_enhance(Yh)
...
class LearnableSharpen(nn.Module):
    def __init__(self, channels):
        super(LearnableSharpen, self).__init__()
    ...
    def forward(self, x):
        x1 = self.conv1(x)
        x3 = self.conv3(x)
        out = self.fuse(torch.cat([x1, x3], dim=1))
        return out

class HFEnhancer_DWT(nn.Module):
    def __init__(self, channels):
        super(HFEnhancer_DWT, self).__init__()
        self.att_lh = EdgeAttention(channels)
        self.att_hl = EdgeAttention(channels)
        self.att_hh = EdgeAttention(channels)

        self.sharpen_lh = LearnableSharpen(channels)
        self.sharpen_hl = LearnableSharpen(channels)
        self.sharpen_hh = LearnableSharpen(channels)

    def forward(self, yh):  # yh 是一个长度为1的列表，shape: [B, C, 3, H, W]
        yh = yh[0]  # 取出高频特征 [B, C, 3, H, W]
        LH = yh[:, :, 0, :, :]  # LH 分量
        HL = yh[:, :, 1, :, :]  # HL 分量
        HH = yh[:, :, 2, :, :]  # HH 分量

        # 分方向增强
        LH_enh = self.sharpen_lh(self.att_lh(LH))
        HL_enh = self.sharpen_hl(self.att_hl(HL))
        HH_enh = self.sharpen_hh(self.att_hh(HH))

        # 拼成 yh 格式，返回 shape: [B, C, 3, H, W]
        yh_enh = torch.stack([LH_enh, HL_enh, HH_enh], dim=2)
        return [yh_enh]  # 返回仍是 list，保持与 DWTForward 对应