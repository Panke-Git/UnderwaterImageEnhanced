...
class UIRPolyKernelSE(nn.Module):
    def __init__(
            self, in_channels=3, out_channels=3, dim=36, bias=False):
        super(UIRPolyKernelSE, self).__init__()
        self.model_name = 'UIRPolyKernelSE'
        ...
        self.bottleneck = SEBlock(int(dim * 2 ** 2))
...
class SEBlock(nn.Module):
    ...
    def forward(self, x):
        b, c, _, _ = x.size()
        y = self.avg_pool(x).view(b, c)  # Squeeze: [B, C, 1, 1] -> [B, C]
        y = self.fc(y).view(b, c, 1, 1)  # Excitation: [B, C] -> [B, C, 1, 1]
