"""
    ...
    @Author: Panke
    @FileName: Unet_HybridAttention_V31.py
    @Time: 2025/12/18 22:52
    @Email: None
"""
...
class UNetHybridAttentionV31(nn.Module):
    """
    基于UNetHybridAttentionV23_2，将HybridAttention替换第一层
    """

    def __init__(self, in_channels=3, out_channels=3, base_c=64):
        super(UNetHybridAttentionV31, self).__init__()
        self.model_name = 'UNetHybridAttentionV31'

        # Down path
        # Layer1
        # self.enc1 = ConvBlock(in_channels, base_c)
        self.hybrid_attention1 = HybridAttention(in_channels)
        ...
        self.conv2 = nn.Conv2d(base_c * 2, base_c, kernel_size=3, padding=1)
...
class HybridAttention(nn.Module):
    def __init__(self, dim, threshold=0.05):
        ...
        self.norm1 = GNConvBlock(in_ch=dim, out_ch=dim, num_groups=1)
        self.norm2 = GNConvBlock(in_ch=dim, out_ch=dim, num_groups=1)
