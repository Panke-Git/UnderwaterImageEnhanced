...
class UNetHybridAttentionV30(nn.Module):
    """
    基于UNetHybridAttentionV23，将HybridAttention中的GN用卷积替代
    """

    def __init__(self, in_channels=3, out_channels=3, base_c=64):
        super(UNetHybridAttentionV30, self).__init__()
        self.model_name = 'UNetHybridAttentionV30'
...
class HybridAttention(nn.Module):
    def __init__(self, dim, threshold=0.05):
        ...
        self.norm1 = GNConvBlock_ConvAblation(in_ch=dim, out_ch=dim)
        self.norm2 = GNConvBlock_ConvAblation(in_ch=dim, out_ch=dim)
...
class DW1x1Affine(nn.Conv2d):
    # 用 depthwise 1x1 conv 模拟 GN 的“每通道缩放+偏置”，参数量完全一致
    def __init__(self, ch: int):
        super().__init__(in_channels=ch, out_channels=ch, kernel_size=1, groups=ch, bias=True)
        nn.init.ones_(self.weight)  # 对齐 GroupNorm 默认 gamma=1
        nn.init.zeros_(self.bias)  # 对齐 GroupNorm 默认 beta=0


class GNConvBlock_ConvAblation(nn.Module):
    def __init__(self, in_ch, out_ch):
        super().__init__()
        self.block = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, kernel_size=3, padding=1, bias=True),
            DW1x1Affine(out_ch),  # ← 替换 GroupNorm(256 params)
            nn.ReLU(inplace=True),
