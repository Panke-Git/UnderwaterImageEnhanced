...
class UNetHybridAttentionV23G(nn.Module):
    """
    基于UNetHybridAttentionV23, 对G进行正则化；
    """

    def __init__(self, in_channels=3, out_channels=3, base_c=64, regularization_type='center'):
        super(UNetHybridAttentionV23G, self).__init__()
        self.model_name = 'UNetHybridAttentionV23G'
        ...
        self.hybrid_attention1 = HybridAttention(base_c * 2, regularization_type=regularization_type)
        ...
        self.hybrid_attention2 = HybridAttention(base_c * 4, regularization_type=regularization_type)
    ...
    def center_constraint(self, G, target=0.5):
        return torch.mean((G - target) ** 2)
    ...
    def forward(self, x):
        ...
        reg1 = self.hybrid_attention1.regularization_loss
        reg2 = self.hybrid_attention2.regularization_loss
        return out, reg1, reg2


class HybridAttention(nn.Module):
    def __init__(self, dim, threshold=0.05, regularization_type='center'):
    ...
    def entropy_regularization(self, G, eps=1e-8):
        G_clamped = torch.clamp(G, eps, 1 - eps)
        entropy = - (G_clamped * torch.log(G_clamped) + (1 - G_clamped) * torch.log(1 - G_clamped))
        return torch.mean(entropy)

    def sparsity_regularization(self, G):
        return torch.mean(torch.abs(G))

    def balance_regularization(self, G):
        mean_G = torch.mean(G)
        return (mean_G - 0.5) ** 2

    def compute_regularization(self, G):
        if self.regularization_type == 'center':
            return self.center_constraint(G)
        elif self.regularization_type == 'entropy':
            return self.entropy_regularization(G)
        elif self.regularization_type == 'sparsity':
            return self.sparsity_regularization(G)
        elif self.regularization_type == 'balance':
            return self.balance_regularization(G)
        else:
            return None
    ...
    def forward(self, x):
        ...
        self.regularization_loss = self.compute_regularization(G)
