...
class UNet_DWT_V2(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, base_c=64):
        """
        将离散小波变换添加到第一个跳跃连接中，然后经过DWT变换，后使用CLACHE处理低频通道，使用增强处理高频通道后输出,其中增加了残差，
        小波变换后加上输入值
        :param in_channels:
        :param out_channels:
        :param base_c:
        """
        super(UNet_DWT_V2, self).__init__()
        self.model_name = 'UNet_DWT_V2'
