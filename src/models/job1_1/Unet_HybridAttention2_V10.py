...
class UNetHybridAttention2V10(nn.Module):
    """
    使用HybridAttention2, 优化低频的部分，插入到Unet的第三层；
    """
    def __init__(self, in_channels=3, out_channels=3, base_c=64):
        super(UNetHybridAttention2V10, self).__init__()
        self.model_name = 'UNetHybridAttention2V10'
...
class LearnableCLAHE(nn.Module):
    def __init__(self, channel):
        super(LearnableCLAHE, self).__init__()
        self.local_conv = nn.Conv2d(channel, channel, kernel_size=3, padding=1, groups=channel)
        self.nonlinear = nn.Sigmoid()

    def forward(self, x):
        local = self.local_conv(x)
        enhanced = self.nonlinear(local)
        return enhanced


class StructureEnhancer(nn.Module):
    def __init__(self, channel):
        super(StructureEnhancer, self).__init__()
        self.conv1 = nn.Conv2d(channel, channel, kernel_size=3, padding=1)
        self.conv2 = nn.Conv2d(channel, channel, kernel_size=5, padding=2)
        self.fuse = nn.Conv2d(channel*2, channel, kernel_size=3, padding=1)

    def forward(self, x):
        feat1 = self.conv1(x)
        feat2 = self.conv2(x)
        out = self.fuse(torch.cat([feat1, feat2], dim=1))
        return out + x

class LLEnhancer(nn.Module):
    def __init__(self, channel):
        super(LLEnhancer, self).__init__()
        self.color_corrector = ColorCorrector(channel)
        self.clahe = LearnableCLAHE(channel)
        self.structure = StructureEnhancer(channel)
