...
class SFGNet(nn.Module):
    ...
    def __init__(self, H=0, W=0, batch_size=0):
        ...
        self.conv1 = nn.Sequential(
            SepConv(in_channel=in_channel, out_channel=n_feats, kernel_size=kernel_size), nn.BatchNorm2d(n_feats),
            nn.GELU()
        )
        self.conv2 = nn.Sequential(
            SepConv(in_channel=n_feats, out_channel=n_feats, kernel_size=kernel_size), nn.BatchNorm2d(n_feats),
            nn.GELU()
        )
