...
# ✅ 替换为你的模型类
from src import models as model  # 请根据你实际模型路径修改

# ----------------- 初始化模型 -----------------
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = model.UNetHybridAttentionV23().to(device)
...
with torch.no_grad():
    for data in tqdm(val_loader):
        input_img, target_img = data  # [B, 3, H, W]
        ...
        for i in range(output.size(0)):  # 支持 batch > 1
            ...
            # ---------- PSNR & SSIM ----------
            pred_np = pred.squeeze(0).permute(1, 2, 0).cpu().numpy()  # HWC
            target_np = target.squeeze(0).permute(1, 2, 0).cpu().numpy()

            psnr = compare_psnr(target_np, pred_np, data_range=1.0)
            ssim = compare_ssim(target_np, pred_np, data_range=1.0, multichannel=True)
...
# ----------------- 输出结果 -----------------
print(f"\n✅ Average PSNR : {total_psnr / count:.4f} dB")
